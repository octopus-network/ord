use super::*;
use pg::{RsTransaction, RsTxIn, RsTxOut, RsUpdates};
...
pub(super) struct RuneUpdater<'a, 'tx, 'client, 'index> {
  ...
  pub(super) rs_updates: &'a mut RsUpdates,
}
...
impl RuneUpdater<'_, '_, '_, '_> {
    ...
    if artifact.is_some() || has_runes {
      ...
      for output in tx.output.iter() {
        ...
        let tx_out = RsTxOut {
          ...
          op_return: if is_op_return { artifact.clone() } else { None },
          runes: vec![],
        };
    ...
    if !self.rs_updates.rs_tx.outputs.is_empty() || !self.rs_updates.rs_tx.inputs.is_empty() {
      ...
      for output in &self.rs_updates.rs_tx.outputs {
        if let Some(ref address) = output.address {
          ...
          self.rs_updates.updated_addresses.insert(address.clone());
        }
      }
  ...
  fn mint(&mut self, id: RuneId) -> Result<Option<Lot>> {
    ...
    self.rs_updates.updated_runes.insert(id);
